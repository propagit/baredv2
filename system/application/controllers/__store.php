...
%s</p>
	...
	function fb()
	{
		 require_once("fb/facebook.php");
		  ...
		  $config['appId'] = '581530448552648';
		  $config['secret'] = '80bf0b8937b6079311b1408c22e8f4b3';
...
".COMPANY_NAME."";
	...
	*/
	...
	function product1($cat_name='',$mcat='') {
		...
		$data['category'] = $cat;
		$data['cat_name'] = $cat_name;
		//$data['products'] = $this->Product_model->get_product_by_category($cat['id']);
		if($this->session->userdata('by'))
		{
			$by = $this->session->userdata('by');
		}
...
// 		
	...
	/*
	function old_products($cat_name='',$type='',$text='',$cm_id=0,$scat='',$limit="",$row="") {
		if($row==""){$row=0;}
		if($limit==""){$limit=12;}
				
		$cat = $this->Category_model->identify2($cat_name);

		$sub2=$this->Category_model->identify_subcat($cm_id);
		if(count($sub2)>0){
		$data['sub_category']=$sub2['name'];}
		else
		{
			$data['sub_category']='';
		}
		...
		if($type=='shop_by')
		{
			$all_products = $this->Product_model->get_product_by_category_shopby($cm_id,$by,$scat);
			$data['products'] = $this->Product_model->get_product_by_category_shopby_pagination($row,$limit,$cm_id,$by,$scat);
		}
		if($type=='style')
		{
			if($text == 'view_all')
			{
				$all_products = $this->Product_model->get_product_by_category_shopby($cm_id,$by,$cat['id']);
				$data['products'] = $this->Product_model->get_product_by_category_shopby_pagination($row,$limit,$cm_id,$by,$cat['id']);
			}
			else 
			{
				$all_products = $this->Product_model->get_product_by_category_style(0,0,$cat['id'],$text,$by);	
				$data['products'] = $this->Product_model->get_product_by_category_style($row,$limit,$cat['id'],$text,$by);	
			}
		...
		if($type=='colour')
		{
			...
			{
				$all_products = $this->Product_model->get_product_by_category_colour(0,0,$cat['id'],$text,$by);	
				$data['products'] = $this->Product_model->get_product_by_category_colour($row,$limit,$cat['id'],$text,$by);	
			}
		...
		if($type=='size')
		{
			...
			{
				$all_products = $this->Product_model->get_product_by_category_size(0,0,$cat['id'],$text,$by);	
				$data['products'] = $this->Product_model->get_product_by_category_size($row,$limit,$cat['id'],$text,$by);	
			}
		}
		if($type=='price')
		{
			...
			{
				$all_products = $this->Product_model->get_product_by_category_price(0,0,$cat['id'],$text,$by);	
				$data['products'] = $this->Product_model->get_product_by_category_price($row,$limit,$cat['id'],$text,$by);	
			}
		...
		//$all_products = $this->Product_model->get_product_by_category_shopby($cm_id,$by,$scat);
		$this->load->library('pagination');
		$config['base_url'] = base_url()."store/products/".$cat_name."/".$type."/".$text."/".$cm_id."/".$scat."/".$limit."/";
		...
		$this->load->view('common/header',$data);
		$this->load->view('store/list_product',$data);
		$this->load->view('common/footer');
	}
	*/
	...
	/*
	function detail_product_test($cat_name='',$id_title='')
	{
		...
		$other = $this->Product_model->get_other_title_product($titles[0],$titles[1],$product['id']);
		//$other_title = $this->Product_model->get_other_product($titles[1],$product['id']);
		$data['like_other']=$other;
		//$other_title = $this->Product_model->get_other_product($titles[1],$product['id']);
		foreach($other as $ot)
		{
			...
			if($hero && $ot['status']==1 && $ot['deleted']==0 && $ot['id']!=$product['id']){
				$all_other[]=$ot['id'];
			}
		}
		$all_other_ori = $all_other;
		...
		if($cc<12){
			$prod12 = $this->Product_model->random_new12($title,$all_other);
			foreach($prod12 as $item)
			{
				if($cc<=12)
				{					
		...
		$this->load->view('common/header',$data);
		//$this->load->view('common/header');
		$this->load->view('store/test_carousel');
		$this->load->view('common/footer');
	}
	*/
...
// 		
	...
	function submit_contact_us()
	{
		...
		$config['mailtype'] = 'html';	
		$this->email->initialize($config);	
		$this->email->from('noreply@bared.com.au','bared.com.au');
