<?php
class New_store extends Controller {
	...
	*/
	...
	function category($action="") {
		if ($action == "add") {
			if (!isset($_POST['title'])) { redirect('admin/new_store/category'); }
			...
			if($title == "") {
				...
				redirect('admin/new_store/category');
			}
	...
	function productgallery($product_id="") {
		...
		$this->load->view('admin/product/new_gallery',$data);
	}
...
// 			
	...
	function makemodal($photo_id)
	{
		...
		{
			$data['modal'] = 0;
			$this->Product_model->update_photo($photo_id,$data);
		}
		else
		{
			$this->Product_model->modal_photo($photo['product_id'],$photo_id);
		}
		
		redirect('admin/new_store/productgallery/'.$photo['product_id']);
	}
