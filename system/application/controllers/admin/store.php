...
class Store extends Controller {
	...
	*/
			...
			foreach($categories as $cat) {
				...
				<tr id="cat-'.$cat['id'].'">
					<td id="detail_cat-'.$cat['id'].'">
						...
						<a href="#anyModal" data-toggle="modal" id="detaillinka'.$cat['id'].'">'.$cat['title'].'</a>
						</span>
	...
	function productgallery($product_id="") {
		...
		$this->load->view('admin/new_product/new_gallery',$data);
	}
	function uploadimage() {
		...
		else
		{
			...
			// $this->advanced_resizephoto($data,"./uploads/products/".$directory,"thumb2",$width,$height,110,120);
			// $this->advanced_resizephoto($data,"./uploads/products/".$directory,"thumb3",$width,$height,767,905);
			// $this->advanced_resizephoto($data,"./uploads/products/".$directory,"thumb4",$width,$height,88,81);
			// $this->advanced_resizephoto($data,"./uploads/products/".$directory,"thumb5",$width,$height,240,180);
			
			$this->resizephoto3($data,"./uploads/products/".$directory,"thumb1",$width,$height,472,515,1);
			$this->resizephoto3($data,"./uploads/products/".$directory,"thumb2",$width,$height,110,120,2);
			$this->resizephoto3($data,"./uploads/products/".$directory,"thumb3",$width,$height,89,97,3);
			$this->resizephoto3($data,"./uploads/products/".$directory,"thumb4",$width,$height,66,73,4);
			$this->resizephoto3($data,"./uploads/products/".$directory,"thumb5",$width,$height,240,262,5);
			
		}
		redirect('admin/store/productgallery/'.$product_id);
	}

	function resizephoto3($data,$directory,$sub,$owidth,$oheight,$width,$height,$no) {
		...
		if ($owidth*$width < $oheight*$height) {		
				...
				//$config['height'] = intval(100 * ($oheight/$owidth) * ($width+10)/($height+10));
			} else {				
				//$config['width'] = intval(100 * ($owidth/$oheight) * ($width+10)/($height+10));
...
// 			
			...
			// }
		// }
			//unlink("./uploads/galleries/thumbs/".$file_name);
			//rename("./uploads/galleries/thumbs/".$data['upload_data']['raw_name']."_thumb".$data['upload_data']['file_ext'],"./uploads/galleries/thumbs/".$file_name);
	}
...
Case Construction Online Merchandise Store
	...
	function addinghtml()
	{
		...
		$new_id = $this->System_model->add_htmlpage($data);
		$ndata['order'] = $new_id;
		$this->System_model->update_html($new_id,$ndata);
	}
	function edit_html($page_id)
	{
		$data['pages'] = $this->System_model->get_detailhtml($page_id);
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');
		$this->load->view('admin/stories/edit_page',$data);		
		$this->load->view('admin/common/footer');
	...
	function updatepagehtml()
	{
		...
		$this->System_model->update_html($id,$data);
		redirect('admin/cms/edit_story/'.$story_id);
	}
	
	function deletehtml()
	{
		$id =  $this->input->post('id');
		$this->System_model->delete_html($id);
	}
	
	function addingpromotionhtml()
	{
		...
		$new_id = $this->Menu_model->add_promotionhtmlpage($data);
		
	}
	function edit_promotionhtml($page_id)
	{
		$data['pages'] = $this->Menu_model->get_detailpromotionhtml($page_id);
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');
		$this->load->view('admin/promotions/edit_page',$data);		
		$this->load->view('admin/common/footer');
		
	}
	function updatepromotionhtml()
	{
		$id =  $this->input->post('id');
		$promotion_id =  $this->input->post('promotion_id');
		$title =  $this->input->post('title');
		...
		$this->Menu_model->update_promotionhtml($id,$data);
		redirect('admin/store/edit_promo/'.$promotion_id);
	}
	
	function deletepromotionhtml()
	{
		$id =  $this->input->post('id');
		$this->Menu_model->delete_promotionhtml($id);
	}
	function preview_promotion($type,$id)
	{
		...
		$data['slide']=-1;
		if($type=='html'){
			$data['pages_story'] = $this->Menu_model->get_promotionpagedetail($id);
		}
		else
		{
		...
		$this->load->view('admin/promotions/promotion',$data);
