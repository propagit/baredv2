...
class System extends Controller {
	...
	function attribute($action="",$attribute_id="") {
		...
		if ($action == "edit") {
			...
			$this->load->view('admin/system/attribute/list',$data);
		}
	...
	function shipping_v2($action="",$shipping_id="") {
		...
		if ($action == "add2") {
			...
			//print_r($data['states']);
			$this->load->view('admin/system/shipping/add2',$data);
		}
		else if ($action == "add") {
			$data['countries'] = $this->System_model->get_countries();
			$data['states'] = $this->System_model->get_states();
	...
	function shipping($action="",$shipping_id="") {
		...
		if ($action == "add") {
			...
			//print_r($data['states']);
			$this->load->view('admin/system/shipping/add_new',$data);
		} else if ($action == "edit") {
			...
			$data['conditions'] = $conditions;
			$this->load->view('admin/system/shipping/edit_new',$data);
		} else {
			$data['shippings'] = $this->System_model->get_shippings();
			//$this->load->view('admin/system/shipping/list',$data);
			$this->load->view('admin/system/shipping/list',$data);
	...
	function change_zone_country($shipping_id)
	{
		$suburbs = $this->System_model->get_country_zone($shipping_id);
		$out='<select name="country_zone" multiple="multiple" id="country_zone" style="height:250px;width:250px;">';
		foreach($suburbs as $st)
		{
			if($st['country_id'] != 238)
			{
				$sb = $this->System_model->get_country1($st['country_id']);
				$out.='<option value="'.$sb['id'].'">'.$sb['name'].'</option>';
			}                                    
		}
        $out.="</select>";
		echo $out;
	...
	function addzonecountry()
	{
		...
		foreach($multiple_subs as $key => $value)
		{
			if($multiple_subs[$key] > 0)
			{
				...
				if(count($check)==0){
					$data=array('state_id' => 0, 'country_id' => $value, 'shipping_id' => $shipping_id);
...
//			 $data=array('zone' => $zone, 'suburb' => $value);
			 ...
			 $this->System_model->delete_zone_country($shipping_id,$value);			
			}
		}	
	}
	...
	function update_shipping()
	{
		...
		if($country_id == 13)
		{
			$this->System_model->delete_shipping_country($shipping_id);
		}
	...
	function duplicate_shipping()
	{
		...
		$data= array(
			...
			'base_rate' => $ship_detail['base_rate'],
			'created' => date('Y-m-d H:i:s')
		);
	...
	function coupon($action="",$coupon_id="") {
		...
		if ($action == "add") {
			...
			$this->load->view('admin/system/coupon/add',$data);
		} else if ($action == "edit") {
			$categories = $this->Category_model->get_main();
			$data['categories']=$categories;
	...
	function search_product()
	{
		...
		$row=0;
		$products = $this->Product_model->search_groupq_no_limit($keyword,0,0,false);
		$out='';
				...
				<div style="width: 70%; float: left; line-height: 50px;">
					If they spend more than &nbsp;&nbsp; 
						<div class="input-prepend">
							...
							<input class="span2" id="valuecondition'.$id.'" name="valuecondition'.$id.'" type="text" style="width:115px!important;">
						</div> &nbsp;&nbsp; 
	...
	function currency() {		
		...
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');
		$this->load->view('admin/system/currency',$data);
		$this->load->view('admin/common/footer');
	}
	...
	function email() {
		...
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');
		$this->load->view('admin/system/email',$data);
		$this->load->view('admin/common/footer');
	}
	...
	function story($coupon_id="")
	{
		...
		$this->load->view('admin/system/story/list',$data);
		$this->load->view('admin/common/footer');
	}
	function add_story()
	{
		...
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');		
		$this->load->view('admin/system/story/add');
		$this->load->view('admin/common/footer');
	}
	function edit_story($id)
	{
		...
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');		
		$this->load->view('admin/system/story/edit',$data);
		$this->load->view('admin/common/footer');
	}
	function activestory($id="") {
		$this->System_model->active_story($id);
		redirect('admin/system/story');
	}
	...
	function add_home_tile()
	{
		...
		if ( ! $this->upload->do_upload()) {
			...
			redirect('admin/system/edit_story/'.$id);	
		}
		else
		{
	...
	function webstat()
	{
		...
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/leftbar');		
		$this->load->view('admin/system/webstat',$data);
		$this->load->view('admin/common/footer');
	}
