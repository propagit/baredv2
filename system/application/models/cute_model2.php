<?php
class Cute_model2 extends Model {
  public $AccessKey;
  public $BackColor;
  public $BorderStyle;
  public $BorderWidth;
...
//********************************************************
  ...
  {
    $this->ActiveTab="Edit";
    $this->AllowEditServerSideCode=true;
    ...
    $this->CodeViewTemplateItemList="Print,Cut,Copy,Paste,Find,ToFullPage,FromFullPage,SelectAll,SelectNone";
    $this->ConvertHTMLTagstoLowercase=false;
    ...
    $this->EditorBodyStyle="white-space:wrap";
    ...
    $this->EditorWysiwygModeCss="/staffmasternew/css/template.css";
    ...
    $this->UsePhysicalFormattingTags=true;
  ...
  function CreateToolBar()
  {
    ...
    {
      $s=$this->GetToolbarItems($this->GetURL("Configuration/AutoConfigure/".$this->AutoConfigure.".config"),$this->DisableItemList);
    } 
    return $s;    
  } 
  ...
  function BuildBottomBar()
  {
    ...
    if (($this->ShowBottomBar))
    {
      ...
      $s=$s."<img title=\"".$this->G_Str("Normal")."\" Command=\"TabEdit\"  src=\"".$this->ProcessThemeWebPath("design.gif")."\" border=\"0\" />";
      if ($this->ShowHtmlMode==true)
      {
        $s=$s."<img title=\"".$this->G_Str("HTML")."\" Command=\"TabCode\"  src=\"".$this->ProcessThemeWebPath("htmlview.gif")."\" border=\"0\" />";
      }
	  if ($this->ShowPreviewMode==true)
	  {
		$s=$s."<img title=\"".$this->G_Str("Preview")."\" Command=\"TabView\"  src=\"".$this->ProcessThemeWebPath("preview.gif")."\" border=\"0\" />";
	  } 
	  $s=$s."</td>";
	  ...
	  {
		$s=$s."<td align='right' style='text-align:right;' nowrap='nowrap'><span class='WordCount'></span><span class='WordSpliter'>&nbsp;</span><span class='CharCount'></span></td>";
	  } 
      $s=$s."<td align='right'>";
	  if (strtolower($this->ResizeMode)=="plusminus")
	  {
		  ...
		  {
			$s=$s."<img title=\"".$this->G_Str("Enlarge")."\" Command=\"sizeplus\" src=\"".$this->ProcessThemeWebPath("plus.gif")."\" border=\"0\"/>";
		  } 

		  if ($this->ShowDecreaseButton)
		  {
			$s=$s."<img title=\"".$this->G_Str("Decrease")."\" Command=\"sizeminus\" src=\"".$this->ProcessThemeWebPath("minus.gif")."\" border=\"0\"/>";
		  } 
	  }
	  else
	  {
		...
		{
			$s=$s."<img title=\"".$this->G_Str("Resize")."\" ondragstart=\"return false\" onmouseover=\"(CuteEditor_GetEditor(this).RegisterResizeCornor||function(){})(this.parentNode)\" src=\"".$this->GetURL("Images/ResizeCorner.gif")."\" border=\"0\"/>";
		}	  
	  }
  ...
  {    	
    ...
    $t=$t."'EditorSetting2':'".$this->EditorSetting2()."',";
  ...
  { 
    ...
    {
      $s="<script language=\"JavaScript\" SRC=\"".$this->FilesPath."/Scripts/Constant.js\"></script><script language=\"JavaScript\" SRC=\"".$this->FilesPath."/license/CuteEditorPHPLic.js\"></script>";
    } 
    return $s;    
  } 
  ...
  function GetString()
  {
    ...
    {
      $s=$s." <!-- CuteEditor Version 6.1 ".$this->ID." Begin --> "."\r\n";
      ...
      $s=$s."<table ".$t." id=\"".$this->ClientID()."\" _IsCuteEditor=\"True\" cellspacing=\"0\" cellpadding=\"0\" height=".$this->Height." width=".$this->Width." style=\"background-color:".$this->BackColor.";border-color:".$this->BorderColor.";border-width:".$this->BorderWidth."px;border-style:".$this->BorderStyle.";table-layout:auto;height:".$this->Height.";width:".$this->Width.";\">";
...
//--------------------------------------	
  ...
  {
    $s="";
    $s=$s.$_SERVER["SERVER_NAME"]."|";
    $s=$s.$_SERVER["HTTP_HOST"]."|";
	$s=$s.gethostbyname($_SERVER["SERVER_NAME"])."|";
    //$s=$s.$_SERVER["REMOTE_HOST"]."|";
  ...
  {		
	$xml;	
	$item;
	$xml=simplexml_load_file( $this->fixURL($this->GetURL("Configuration/Security/".$this->SecurityPolicyFile."") ));
	if ($xml == false) {
  ...
  function LoadHTML($FilePath)
  {
    ...
    { 
		$xml = simplexml_load_file($this->fixURL($this->GetURL("Languages/".$culture.".xml")));    
		if ($xml == false) {
		...
		$xml = simplexml_load_file($this->fixURL($this->GetURL("Languages/en-en.xml")));     
	...
	switch (strtolower($this->ThemeType))
	{
	  case "office2003_bluetheme":
		return $this->GetURL("Themes/Office2003_BlueTheme/Images/".$imageURL);
		break;
	  case "office2007":
	  ...
	  case "officexp":
		...
		break;
	  case "office2000":
		return $this->GetURL("Themes/Office2000/Images/".$imageURL);
		break;
	  case "custom":
		return $this->GetURL("Themes/Custom/Images/".$imageURL);
		break;
	  default:
		return $this->GetURL("Themes/Custom/Images/".$imageURL);
		break;
	...
	{
		if ($this->BrowserType=="Gecko" || $this->BrowserType=="Opera")
		{
		  $d_list=$d_list.",zoom";
		} 
		...
		$s="";
		$xml = simplexml_load_file($this->fixURL($this->GetURL("Configuration/AutoConfigure/Full.config")));
		if ($xml == false) {
	...
	function AddToolbarDropDownfromConfig($name,$command,$width)
	{
		$s="";	
		$xml="";	
		$xml=simplexml_load_file( $this->fixURL($this->GetURL("Configuration/Shared/Common.config")) );
	...
	{	  
	  ...
	  $str_temp="<img id='".$c."' Command=\"ForeColor\" title=\"".$this->G_Str("ForeColor")."\" src='".$this->ProcessThemeWebPath("fontcolor.gif")."' width='17' height='20' border=0 style='background-color: red;' ".$t."/>";
	  $t="";
	  if ($this->ReadOnly!=true)
	  {
	  ...
	  return $str_temp."<img Command=\"SetForeColor\" title=\"".$this->G_Str("SetForeColor")."\" src='".$this->ProcessThemeWebPath("tbdown.gif")."' width='9' height='20' border='0' ".$t."/>";
	...
	{ 
	  ...
	  $str_temp="<img id='".$c."' Command=\"BackColor\" title=\"".$this->G_Str("BackColor")."\" src='".$this->ProcessThemeWebPath("colorpen.gif")."' width='17' height='20' border=0 style='background-color: yellow;' ".$t."/>";
	  $t="";
	  if ($this->ReadOnly!=true)
	  {
	  ...
	  return $str_temp."<img Command=\"SetBackColor\" title=\"".$this->G_Str("SetBackColor")."\" src='".$this->ProcessThemeWebPath("tbdown.gif")."' width='9' height='20' border='0' ".$t."/>";
    ...
    {    
      ...
      return "<img Command=\"TableDropDown\" title=\"".$this->G_Str("TableDropDown")."\" src='".$this->ProcessThemeWebPath("instable.gif")."' ".$t."/>";      
    } 
