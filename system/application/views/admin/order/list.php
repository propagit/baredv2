...
function export_eparcel_csv(type){
	if (confirm('This will export the order list to a ePacel csv file. Do you want to continue?')) {
		window.location = '<?=base_url()?>admin/order/export_eparcel_csv/'+type;
	}
}
		...
		<div style="padding: 20px">
			...
			<div>
				...
				<div style="float: left; width: 80%">
					<input type="text" class="textfield rounded" id="name" name="customer_name" value="<?=isset($search_params['customer_name']) ? $search_params['customer_name'] : '';?>"/>
				</div>
			</div>
			<div>
				<div style="float: left; width: 20%; height: 30px; line-height: 30px">
				...
				<div style="float: left; width: 80%">
					<input type="text" class="textfield rounded" id="name" name="customer_id" value="<?=isset($search_params['customer_id']) ? $search_params['customer_id'] : '';?>"/>
				</div>
			</div>
			<div>
				<div style="float: left; width: 20%; height: 30px; line-height: 30px">
				...
				<div style="float: left; width: 80%">
					<input type="text" class="textfield rounded" id="name" name="order_id" value="<?=isset($search_params['order_id']) ? $search_params['order_id'] : '';?>"/>
				</div>
			</div>
			<div>
				<div style="float: left; width: 20%; height: 30px; line-height: 30px">
				...
				<div style="float: left; width: 80%">
					<div id="datetimepicker1" class="input-append">
					    <input data-format="dd-MM-yyyy" type="text" name="from_date" value="<?=isset($search_params['from_date']) ? $search_params['from_date'] : '';?>"></input>
			...
			<div>
				...
				<div style="float: left; width: 80%">
					<div id="datetimepicker4" class="input-append">
					    <input data-format="dd-MM-yyyy" type="text" name="to_date" value="<?=isset($search_params['to_date']) ? $search_params['to_date'] : '';?>"></input>
					    <span style="cursor: pointer" class="add-on">
					      <i data-time-icon="icon-time" data-date-icon="icon-calendar">
					      </i>
					    </span>
			...
			<div>
				...
				<div style="float: left; width: 80%">
					<select class="selectpicker" id="stat" name="by_status">
						<option value="All" <?=$search_params['by_status'] == 'All' ? 'selected="selected"' : '';?>>All</option>
						<option value="failed" <?=$search_params['by_status'] == 'failed' ? 'selected="selected"' : '';?>>Failed</option>
                        <option value="cancelled" <?=$search_params['by_status'] == 'cancelled' ? 'selected="selected"' : '';?>>Cancelled</option>
						<option value="shipped" <?=$search_params['by_status'] == 'shipped' ? 'selected="selected"' : '';?>>Shipped</option>
						<option value="processed" <?=$search_params['by_status'] == 'processed' ? 'selected="selected"' : '';?>>Processed</option>
						<option value="increasing status" <?=$search_params['by_status'] == 'increasing status' ? 'selected="selected"' : '';?>>Increasing Status</option>
						<option value="30 days invoice" <?=$search_params['by_status'] == '30 days invoice' ? 'selected="selected"' : '';?>>30 Days Invoice</option>
					</select> 
                    <!-- <script>jQuery(".selectpicker").selectpicker();</script> -->
				</div>
			</div>
			...
			<div>
				...
				<div style="float: left; width: 80%">
					<input type="text" class="textfield rounded" id="name" name="by_keyword" value="<?=isset($search_params['by_keyword']) ? $search_params['by_keyword'] : '';?>"/>
				</div>
			</div>
			<div style="clear: both"></div>
			<div>
            ...
            <div style="float: left; margin-top: 15px; margin-left:15px;">
				<button  onClick="export_eparcel_csv('');" class="btn btn-primary">Export eParcel CSV</button>
			</div>
