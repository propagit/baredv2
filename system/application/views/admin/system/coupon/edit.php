...
function remove_cond(id)
{
	...
	jQuery.ajax({
		...
		success: function(html) {		
			jQuery('#cond'+id).remove();
			update_coupon();//location.reload();
		}
	});
}

function removes_cond(id,i)
{
	jQuery('#cond'+id).remove();
	jQuery.ajax({
		url: '<?=base_url()?>admin/system/remove_condition/'+id,
...
{
	//alert('test');
	if (jQuery(".head_category"+id).is(":checked")) {		
		jQuery('.detail_category_cb'+id).prop('checked', true);
	}
	else
	{
		jQuery('.detail_category_cb'+id).prop('checked', false);
	}
}
/*
function save_category()
...
function update_coupon()
{
	...
	jQuery.ajax({
		url: '<?=base_url()?>admin/system/updatecoupon/',
		type: 'POST',	
		...
		success: function(html) {		
			...
			$('#anyModal').on('hidden', function () {
  				window.location='<?=base_url()?>admin/system/coupon/edit/'+idcoupon;
			})
			...
			<form method="post" action="<?=base_url()?>admin/system/updatecoupon">
				...
				<div id="date_range" style="display:none;">
                    <div>
                        ...
                        <? 
						if($coupon['from_date']=='0000-00-00'){$date_from='';}else{
						$date_from=date('d-m-Y',strtotime($coupon['from_date']));} ?>
                        <div style="width: 80%; float: right">
                              <div id="datetimepicker4" class="input-append">
                              
                                <input data-format="dd-MM-yyyy" type="text" name="from_date" id="from_date" value="<? echo $date_from?>"></input>
                    ...
                    <div>
                        ...
                        <div style="width: 80%; float: right">
                            ...
                            <? 
						if($coupon['to_date']=='0000-00-00'){$date_to='';}else{
						$date_to=date('d-m-Y',strtotime($coupon['to_date']));} ?>
                        
                              <div id="datetimepicker1" class="input-append">
                                <input data-format="dd-MM-yyyy" type="text" name="to_date" id="to_date" value="<? echo $date_to?>"></input>
                                <span style="cursor: pointer" class="add-on">
                                  <i data-time-icon="icon-time" data-date-icon="icon-calendar">
                                  </i>
                                </span>
				...
				<div>
					 ...
					 <div style="width: 80%; float: right">
                        ...
                        <select id="condition" style="display:none;"><option <?php if($coupon['type_cond'] == 1) print ' selected="selected"'; ?>value="1">ALL</option>
                        
                        <!--<option <?php if($coupon['type_cond'] == 2) print ' selected="selected"'; ?>value="2">ANY</option>-->
                        </select>
					...
					foreach($coupon_condition as $cc)
					{
                                ...
                                <div style="width: 70%; float: left; line-height: 50px;">
                                    If they spend more than &nbsp;&nbsp; 
                                        <div class="input-prepend">
                                            ...
                                            <input class="span2" id="valuecondition<?=$i?>" name="valuecondition<?=$i?>" type="text" value="<?=$cc['value']?>" onblur="save_spend(<?=$i?>)" style="width:115px!important;">
                                        </div> &nbsp;&nbsp; 
                                            Enter a number in format 00.00
                                </div>
                                <div style="float: left; line-height: 50px; margin-top:10px;cursor:pointer;" onclick="removes_cond(<?=$cc['id']?>,<?=$i?>);">
						...
						if($cc['type']==2)
						{
							?>
                                ...
                                <div style="width: 20%; float: left; height: 30px; line-height: 50px;padding-left:10px;">Category Discount <input type="hidden" id="type_cond<?=$i?>" value="2"> <input type="hidden" id="id_type_cond_cat" value="<?=$cc['id']?>"></div>
                                <div style="width: 70%; float: left; line-height: 50px;">
                                        <div>
						...
						if($cc['type']==3)
						{
							?>
                                ...
                                <div style="width: 70%; float: left; line-height: 50px;">
                                        ...
                                        <select class="selectpicker" id="product_type">
                                         	<option value="in" <? if($cc['cond_prod']=='in'){echo "selected='selected'";}?>>in</option>
                                        	<option value="out" <? if($cc['cond_prod']=='out'){echo "selected='selected'";}?>>out</option>
                                        </select></div>
                                        <script>jQuery(".selectpicker").selectpicker();</script>
                                        <style>#category_type{margin-top:10px!important;}#cond'.$id.'{min-height:200px;}</style>
                                               ...
                                               <?  $cat_ids=$cc['products'];
												...
												?>                                                           
                                                                <tr class="detailtrp" id="dtrp<?=$pro_detail['id']?>">
                                                                    <td align="center"><?=$pro_detail['title']?> <?=$pro_detail['short_desc']?></td>
                                                                    <td class="remove">
    ...
    <tbody id="all_page">
        ...
        <? $sub_categories = $this->Category_model->get($ct['id']);
		   foreach($sub_categories as $sc)
		   {
				...
				<td class="cb"><input type="checkbox" value="<?=$sc['id']?>" class="single_category detail_category_cb<?=$ct['id']?>" id="single_category<?=$sc['id']?>" <? if($check_cats==1){?> disabled="disabled" <? }?>></td>
				<td style="text-align: left;">
		 ...
		 if($ct['id']==4){?>
		 		<tr id="detail_category_tr<?=$ct['id']?>" class="detail_category<?=$ct['id']?> detail_cat" style="display:none;">
                <? $check_cats = $this->System_model->check_category_coupon($ct['id'],$coupon['id']);?>
				<td class="cb"><input type="checkbox" value="<?=$ct['id']?>" class="single_category detail_category_cb<?=$ct['id']?>" id="single_category<?=$ct['id']?>" <? if($check_cats==1){?> disabled="disabled" <? }?>></td>
				<td style="text-align: left;">
					<span class="mleft">Sale</span>
				</td>
				<td class="remove" style="display:none;"><div style="cursor:pointer;" onclick="remove_category_cond(<?=$ct['id']?>);"><i style="color: #c70520" class="icon-remove-circle icon-2x"></i></div></td>                 
				</tr>
         <?
		 }
         } 
    ...
    <table class="table table-hover">
            ...
            <tbody id="list_products">
                ...
                <tr id="product-<?=$product['id']?>" class="detail_prod">
                    ...
                    <td style="text-align: left;">
                        <a href="<?=base_url()?>admin/cms/product/edit/<?=$product['id']?>" target="_blank"><?=$product['title']?> <?=$product['short_desc']?></a>
                    </td>
