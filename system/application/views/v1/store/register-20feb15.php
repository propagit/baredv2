...
{		
	...
	if(valid)
	{
		...
		if(email!='' && firstname!='' && lastname!='' && password!='' && state!='' && phone!='')
		{
		
			jQuery.ajax({
				url: '<?=base_url()?>store/traderegister2',
				type: 'POST',
				data: ({firstname:firstname,lastname:lastname, month_dob:month_dob, date_dob:date_dob, email:email, password:password, address1:address1, address2:address2, suburb:suburb, country:country, state:state, postcode:postcode,phone:phone,mobile:mobile,heardus:heardus,personal_referral:personal_referral}),
				dataType: "html",
